...
namespace GridSoul.Controllers
{
    public class GameManager
    {
        ...
        public void HandleInput(Keys key, int width, int height)
        {
            if (key == Keys.Escape)
            {
                ...
                else if (State == GameState.LevelSelect)
                {
                    DialogResult result = MessageBox.Show(
                        "Вы действительно хотите покинуть арену и выйти из игры?",
                        "Выход",
            ...
            if (key == Keys.Enter)
            {
                if (State == GameState.Start || State == GameState.Menu)
                {
                ...
                else if (State == GameState.Intro || State == GameState.LevelTransition || State == GameState.Outro)
                {
                    ContinueStory(width, height);
                }
            }

            if (State == GameState.Shop)
            {
                if (key == Keys.D1 || key == Keys.NumPad1) BuyFireRate();
                if (key == Keys.D2 || key == Keys.NumPad2) BuyMultiShot();
                if (key == Keys.D3 || key == Keys.NumPad3) BuyVampirism();
                if (key == Keys.D4 || key == Keys.NumPad4) BuyDash();
                return;
            }
            ...
            if (CampaignLevel == 5)
            {
                if (key == Keys.Q || key == Keys.D1 || key == Keys.NumPad1)
                    CurrentWeapon = WeaponType.Pilum;

                if (key == Keys.E || key == Keys.D2 || key == Keys.NumPad2)
        ...
        public void MovePlayer(bool up, bool down, bool left, bool right, int width, int height)
        {
        ...
        private void MoveEnemyAroundWalls(Enemy enemy)
        {
            ...
            RectangleF originalBounds = enemy.Bounds;
            ...
            if (bestScore == float.MaxValue || bestBounds == originalBounds)
            {
                PointF[] fallback =
                {
                    new PointF(1, 0),
                ...
                foreach (var dirCandidate in fallback)
                {
                    PointF dir = dirCandidate;
                    Normalize(ref dir);
                    ...
                    enemy.Bounds = future;
                    enemy.Velocity = new PointF(dir.X * enemy.Speed, dir.Y * enemy.Speed);
                    return;
                }
        ...
        {
            int praetorianCount = 0;
            foreach (var enemy in enemies)
            {
                if (enemy.Type == EnemyType.Praetorian)
                    praetorianCount++;
            }

            int maxNewPraetorians = 5 - praetorianCount;
            if (maxNewPraetorians <= 0)
                return;

            SizeF size = GetEnemySize(EnemyType.Praetorian);
            var spawnPoints = new List<PointF>();

            for (int i = 0; i < maxNewPraetorians && i < 2; i++)
            {
                spawnPoints.Add(GetCentralReinforcementSpawn(size));
            }

            foreach (var spawn in spawnPoints)
            {
                PointF safeSpawn = AdjustSpawnPointOutsideWall(spawn, size);
                enemies.Add(CreateEnemy(EnemyType.Praetorian, safeSpawn));
            }
        }

        private PointF AdjustSpawnPointOutsideWall(PointF spawn, SizeF size)
        {
            RectangleF spawnBounds = new RectangleF(spawn.X, spawn.Y, size.Width, size.Height);
            int attempts = 0;

            while (IntersectsAnyWall(spawnBounds) && attempts < 3)
            {
                PointF arenaCenter = GetMapCenter();
                PointF direction = new PointF(arenaCenter.X - spawnBounds.X, arenaCenter.Y - spawnBounds.Y);
                Normalize(ref direction);
                spawnBounds.X += direction.X * 60;
                spawnBounds.Y += direction.Y * 60;
                attempts++;
            }

            return new PointF(spawnBounds.X, spawnBounds.Y);
        }

        private PointF GetCentralReinforcementSpawn(SizeF size)
        {
            if (mapAreas.Count == 0)
                return GetMapCenter();

            RectangleF arena = mapAreas[0];
            float minX = arena.Left + arena.Width / 4f;
            float maxX = arena.Right - arena.Width / 4f - size.Width;
            float minY = arena.Top + arena.Height / 4f;
            float maxY = arena.Bottom - arena.Height / 4f - size.Height;

            int xMin = (int)Math.Round(minX);
            int xMax = (int)Math.Round(Math.Max(xMin + 1, maxX));
            int yMin = (int)Math.Round(minY);
            int yMax = (int)Math.Round(Math.Max(yMin + 1, maxY));

            float x = rand.Next(xMin, xMax);
            float y = rand.Next(yMin, yMax);
        ...
        private void SpawnCampaignEnemy(int width, int height)
        {
            ...
            if (CampaignLevel == 5)
            {
                RectangleF testBounds = new RectangleF(pos.X, pos.Y, GetEnemySize(currentEnemyType).Width, GetEnemySize(currentEnemyType).Height);
                if (IntersectsAnyWall(testBounds))
                {
                    PointF center = new PointF(width / 2f, height / 2f);
                    PointF dir = new PointF(center.X - pos.X, center.Y - pos.Y);
                    Normalize(ref dir);
                    pos.X += dir.X * 50;
                    pos.Y += dir.Y * 50;
                }
        ...
        public void Shoot(float dx, float dy)
        {
            ...
            if (CurrentWeapon == WeaponType.Gladius || CurrentWeapon == WeaponType.Knife)
            {
                ...
                // Жесткое ограничение скорости атаки ножа на 1 уровне
                if (CurrentWeapon == WeaponType.Knife && CampaignLevel == 1)
                    shootCooldown = 25; // ~500мс при таймере 20ms
                else
        ...
        private void UpdateSlashesAfterCombat()
        {
            UpdateSlashes();
        }
